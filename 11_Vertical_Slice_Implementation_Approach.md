...
**Item 8 is now implemented — Slice v1 is complete, closing out the full [08 §13](08_Vertical_Slice_Specification.md#13-definition-of-done) definition of done.** `ScenarioSaveService` (`Assets/Scripts/CityAuthority/SaveLoad/`) captures every runtime system's state to a JsonUtility-serializable `ScenarioSaveData` and restores it via additive `Restore()` factories added to each runtime class (`CityLog`, `EmergencyIncidentRuntime`, `CondemnationCaseRuntime`, `DevelopmentProposalCycleRuntime`, `NewspaperCoverageRuntime`) rather than replaying anything — the court ruling, published articles, and City Log entries all come back as stored data, never recomputed. The one exception is the final report itself: it's a pure, deterministic read of the other restored state (per Section 2's "read the graph" pattern), so it's safely recomputed on load rather than also being persisted separately. Verified with a live Play mode round trip through `EmergencyDebugPanel`'s Save/Load buttons: save mid-scenario, progress further, reload, and confirm state reverts to exactly the saved snapshot.
