...
- The active build target is the **vertical slice** (Report 08), built per its suggested build order (08 §14, amended by 11 §2-§3). **All seven steps are implemented; Slice v0 (11 §5) is complete.**
  ...
  All seven are wired together in `Assets/Data/Slice/SliceConfig_Default.asset` and drivable by hand via the bare-bones `EmergencyDebugPanel` (`Assets/Scripts/CityAuthority/DebugUI/`), attached to a GameObject in `Assets/Scenes/SampleScene.unity` — press Play and use its buttons to walk the whole scenario (emergency → condemnation → ruling → development decision → newspaper → final report). Each step has EditMode tests under `Assets/Tests/Editor/`.

  Save/load reproducibility (08 §13 item 8, slice v1 per 11 §5) remains the only deferred item from the full Report 08 definition of done.
