...
- **The first real player-facing UI has landed**: a UGUI left-side toggle menu (Cities-Skylines-style category bar, positioned left instead of bottom), covering six categories (Emergency, Save/Load, Development, Court, Media, Reports) — Emergency and Save/Load are fully wired to real game state; the other four show a "Coming soon" placeholder. Code lives in two new folders:
  - `Assets/Scripts/CityAuthority/Session/` — `ScenarioSession` (plain C#) extracts everything `EmergencyDebugPanel` used to own (bootstrap + all action methods + session-level flags like `WarningResponded`/`FinalReport`) out from behind `MonoBehaviour.Awake()`, and `ScenarioSessionHost` (thin `MonoBehaviour`) exposes one shared instance in the scene. **Both `EmergencyDebugPanel` and the new UGUI menu read/act through the same `ScenarioSessionHost.Session` reference** — this is why they never drift out of sync when both are open at once, and it's also what made the orchestration flow unit-testable for the first time (`ScenarioSessionTests.cs`) independent of any `MonoBehaviour`.
  - `Assets/Scripts/CityAuthority/UI/` — `MenuController` (tracks the active category, shows/hides submenu roots), `CategoryMenuButton` (wraps a UGUI `Toggle`), `EmergencySubmenuView`/`SaveLoadSubmenuView` (the two wired categories — each polls `Session` in `Update()` while its submenu is active so the debug panel and menu can never show contradictory state), `ComingSoonSubmenuView` (placeholder, reused for the other four).
  - Scene additions: `ScenarioSession` (host GameObject), `EventSystem` + `InputSystemUIInputModule` (required — this project's `activeInputHandler` is Input System only, the legacy `StandaloneInputModule` won't receive input), `Canvas` (`Screen Space - Overlay`) containing `LeftMenuBar` and `SubmenuPanel`. `EmergencyDebugPanel`'s IMGUI window was shifted to `x=480` so it no longer overlaps the new menu on screen; it's kept alive on purpose as a parallel dev tool, not retired.
  - TextMeshPro's essential resources were imported (`Assets/TextMesh Pro/`) since this is real, ongoing UI — all new text uses `TextMeshProUGUI`, not legacy `Text`.
...
- Tegan confirmed (2026-08-03, per [12 §3 item 1](12_External_Review_and_Recommended_Next_Steps.md#3-open-questions-for-tegan-do-not-resolve-unilaterally)) that this dated worktree/session checkout is treated as canonical — dated session-style folder names alone aren't a signal of staleness.
...
- **A `MonoBehaviour`'s `Start()` may never fire when a GameObject is toggled from inactive to active via an MCP `RunCommand` script** (e.g. `someButton.Toggle.isOn = true` triggering `gameObject.SetActive(true)` on a submenu root that had been inactive since scene load) — `Awake()`/fields populate fine, `Time.frameCount` keeps advancing normally, no exception is thrown or logged, but `Start()`'s body (registering `onClick` listeners, populating text) silently never runs, even after several real seconds. Confirmed via `GameObject.SendMessage("Start", SendMessageOptions.DontRequireReceiver)`, which immediately runs it correctly — the component and its wiring are fine, only Unity's own deferred-Start dispatch is failing to fire in this specific MCP-driven context. `OnEnable()`-based init doesn't have this problem (it fires synchronously with `SetActive`, confirmed working every time in the same session) — prefer `OnEnable()` over `Start()` for anything that needs to run on first activation of an object that starts inactive, if you want it verifiable via RunCommand without a manual `SendMessage("Start", ...)` primer. This has only been observed when *automating* activation via script; there's no reason to expect it affects a real player's mouse-driven interaction, since that goes through the normal `EventSystem` → `Update()` path.
